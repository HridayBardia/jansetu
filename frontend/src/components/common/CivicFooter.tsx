...
export const CivicFooter: React.FC = () => {
  ...
  return (
    <footer 
      role="contentinfo"
      aria-label="National Portal Civic Footer"
      className="w-full bg-slate-100 dark:bg-slate-950 border-t border-slate-300 dark:border-slate-800 text-slate-600 dark:text-slate-400 text-xs py-8 px-4 md:px-12 transition-colors select-none font-sans mt-auto"
    >
      <div className="max-w-7xl mx-auto space-y-6">
        {/* Top Tier: Official Mandatory Policy Links (GIGW 3.0 & DPDP Act Standards) */}
        <nav aria-label="Official Mandatory Policy Directory">
          <ul className="flex flex-wrap items-center justify-center md:justify-start gap-x-6 gap-y-2.5 border-b border-slate-200 dark:border-slate-800 pb-5 font-medium text-slate-700 dark:text-slate-300 list-none p-0 m-0">
            <li>
              <Link 
                href="/legal/website-policies" 
                className="hover:text-blue-800 dark:hover:text-blue-400 transition hover:underline focus:outline-hidden focus:ring-2 focus:ring-blue-600 rounded-sm py-0.5"
              >
                {t('footer.websitePolicies', 'Website Policies')}
              </Link>
            </li>
            <li aria-hidden="true" className="text-slate-300 dark:text-slate-700 select-none">|</li>
            ...
            <li>
              <Link 
                href="/legal/hyperlinking-policy" 
                className="hover:text-blue-800 dark:hover:text-blue-400 transition hover:underline focus:outline-hidden focus:ring-2 focus:ring-blue-600 rounded-sm py-0.5"
              >
            ...
            <li>
              <a 
                ...
                title="Open National Portal of India in new window"
              >
        ...
        {/* Bottom Tier: Copyright & Gazette Release */}
        <div className="pt-3 border-t border-slate-200 dark:border-slate-800/80 flex flex-col sm:flex-row items-center justify-between gap-2 text-[11px] text-slate-500 dark:text-slate-400">
          ...
          <p className="font-mono">Last Updated: August 2026 • Build Release v3.0-GIGW-DPDP</p>
        </div>
      </div>
    </footer>
  );
