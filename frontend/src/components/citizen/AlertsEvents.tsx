...
export const AlertsEvents: React.FC = () => {
  ...
  const handleShareFromVaultItem = (alertItem: any) => {
    ...
    setTimeout(() => {
      ...
      setSuccessMsg(t(`✓ Verified ${docName} shared with ${targetDept}.`, `✓ Verified ${docName} shared with ${targetDept}.`));
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setTimeout(() => {
      ...
      setSuccessMsg(t(`✓ Attested document file "${file.name}" uploaded and transmitted to ${targetDept}.`, `✓ Attested document file "${file.name}" uploaded and transmitted to ${targetDept}.`));
      setTimeout(() => {
        setSuccessMsg(null);
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            <h2 className="text-base font-bold text-slate-900 dark:text-white flex items-center gap-2">
              ...
              <span>{t('Regulatory Policy Triggers', 'Regulatory Policy Triggers')}</span>
            </h2>
            <p className="text-xs text-slate-600 dark:text-slate-400 mt-0.5">
              {t('Official regulatory policy triggers automatically mapped to your location and profile.', 'Official regulatory policy triggers automatically mapped to your location and profile.')}
            </p>
          </div>
          ...
          <button
            ...
            title={t('Refresh mapped regulatory policies', 'Refresh mapped regulatory policies')}
          >
              ...
              >
                <div className="flex justify-between items-center text-xs">
                  <span className={`font-bold border px-2 py-0.5 rounded text-[11px] ${tagBg}`}>
                    {t(policy.tag, policy.tag)}
                  </span>
                  <span className="text-[10px] text-slate-500 font-mono">{t(policy.effectiveDate, policy.effectiveDate)}</span>
                </div>
                <h4 className="text-xs font-bold text-slate-900 dark:text-white">{t(policy.title, policy.title)}</h4>
                <p className="text-[11px] text-slate-600 dark:text-slate-400 leading-relaxed">{t(policy.description, policy.description)}</p>
              </div>
            );
          })}
        </div>
        ...
        <div className="space-y-3">
          {webhookAlerts.length === 0 ? (
            <div className="bg-slate-50 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-700 p-6 rounded-xl text-center text-slate-500 text-xs space-y-1">
              ...
              <p className="text-[11px] text-slate-400">{t('Real-time alerts, status updates, and department e-KYC requests will appear here when requested for your account.', 'Real-time alerts, status updates, and department e-KYC requests will appear here when requested for your account.')}</p>
            </div>
          ) : (
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-2">
                        ...
                        <h4 className="text-amber-900 dark:text-amber-300 font-bold text-xs">
                          {t('Action Required: Department Request', 'Action Required: Department Request')}
                        </h4>
                      </div>
                      <span className="text-[10px] text-slate-500 font-mono font-bold">{t(item.timestamp || 'Just now', item.timestamp || 'Just now')}</span>
                    </div>

                    <p className="text-xs text-slate-800 dark:text-slate-200 leading-relaxed">
                      <strong>{t(item.deptName || item.dept || 'Ministry of Education', item.deptName || item.dept || 'Ministry of Education')}</strong> {t('requested a verified copy for', 'requested a verified copy for')} <strong>{t(docName, docName)}</strong> ({t('App', 'App')} #{item.appId || 'JS-2026-8802'}).
                    </p>
                    ...
                    {vaultCheck.isInVault ? (
                      <div className="py-2 px-3 rounded-xl bg-emerald-100/90 dark:bg-emerald-950/80 border border-emerald-300 dark:border-emerald-700 flex items-center justify-between text-xs text-emerald-900 dark:text-emerald-300 font-semibold">
                        <span className="flex items-center gap-1.5">
                          ...
                          <span>{t('✓ Verified record located in your DigiLocker Vault', '✓ Verified record located in your DigiLocker Vault')}</span>
                        </span>
                        <span className="text-[10px] bg-emerald-600 text-white font-bold px-2 py-0.5 rounded shadow-xs">{t('8 Verified Files', '8 Verified Files')}</span>
                      </div>
                    ) : (
                      <div className="py-2 px-3 rounded-xl bg-amber-100/90 dark:bg-amber-950/80 border border-amber-300 dark:border-amber-700 flex items-center justify-between text-xs text-amber-900 dark:text-amber-200 font-semibold">
                        ...
                        <span className="text-[10px] bg-amber-600 text-white font-bold px-2 py-0.5 rounded shadow-xs">{t('Upload Required', 'Upload Required')}</span>
                      </div>
                    )}
                      ...
                      {vaultCheck.isInVault ? (
                        <>
                          ...
                          >
                            ...
                            <span>📤 {t('Upload Manual Copy', 'Upload Manual Copy')}</span>
                          </button>
                        </>
                      ) : (
                        <button
                  ...
                  >
                    ...
                    <div className="space-y-1 flex-1">
                      <div className="flex justify-between items-center">
                        <span className="font-bold text-emerald-900 dark:text-emerald-200">{t('e-KYC Verification Completed', 'e-KYC Verification Completed')}</span>
                      ...
                      <p className="text-[11px] text-emerald-800 dark:text-emerald-300 leading-relaxed font-medium">
                        {item.resolvedText || `✓ ${t('Verified', 'Verified')} ${t(item.requestedDoc || 'Polytechnic Marksheet', item.requestedDoc || 'Polytechnic Marksheet')} ${t('shared with', 'shared with')} ${t(item.deptName || 'Ministry of Education', item.deptName || 'Ministry of Education')}.`}
                      </p>
                    </div>
                  </div>
                );
