      ...
      >
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="text-[10px] font-black uppercase tracking-wider bg-rose-900/60 px-2 py-0.5 rounded border border-rose-400/40">
                {t('Security Alert', 'Security Alert')}
              </span>
            ...
            <h2 id="session-break-title" className="text-base font-extrabold tracking-tight mt-0.5 text-white">
              {t('Session Terminated: Concurrent Login', 'Session Terminated: Concurrent Login')}
            </h2>
          </div>
        </div>
        ...
        <div className="p-6 space-y-4 text-xs">
          <div className="p-3.5 bg-rose-50 dark:bg-rose-950/40 border border-rose-200 dark:border-rose-800 rounded-xl flex items-start gap-3">
            ...
            <div className="space-y-1">
              <span className="font-bold text-rose-900 dark:text-rose-200 text-xs">
                {t('Active Session Overwritten', 'Active Session Overwritten')}
              </span>
              <p className="text-rose-700 dark:text-rose-300 text-[11px] leading-relaxed">
                {t('Your account', 'Your account')} (<span className="font-bold text-rose-950 dark:text-rose-100">{accountName}</span>) {t('was just logged into on another browser tab or device.', 'was just logged into on another browser tab or device.')}
              </p>
            </div>
          </div>
          ...
          <div className="space-y-2 text-slate-600 dark:text-slate-300 leading-relaxed text-[11px]">
            <p>
              {t('In accordance with Digital Personal Data Protection (DPDP) Act 2023 single-session integrity protocols, active telemetry and authenticated access on this tab have been disconnected to prevent unauthorized dual access.', 'In accordance with Digital Personal Data Protection (DPDP) Act 2023 single-session integrity protocols, active telemetry and authenticated access on this tab have been disconnected to prevent unauthorized dual access.')}
            </p>
          </div>
          ...
          <div className="flex items-center gap-2 p-2.5 bg-slate-50 dark:bg-slate-800/60 rounded-lg border border-slate-200 dark:border-slate-700 text-[10px] text-slate-500 dark:text-slate-400">
            ...
            <span>{t('All local session credentials and tokens for this tab have been purged.', 'All local session credentials and tokens for this tab have been purged.')}</span>
          </div>
          ...
          <div className="pt-2">
            ...
            >
              ...
              <span>{t('Return to Sign In Page', 'Return to Sign In Page')}</span>
            </button>
          </div>
        </div>
      </div>
