  ...
  return (
    <div className="relative" ref={dropdownRef} onKeyDown={handleKeyDown}>
      ...
      <button
        ...
        className="flex items-center gap-1.5 bg-slate-900 border border-slate-800 rounded-lg px-2.5 py-1.5 text-xs text-slate-200 focus-within:border-amber-500/50 hover:border-slate-700 transition"
      ...
      >
        ...
        {translationStatus === 'degraded' && (
          <span className="w-1.5 h-1.5 bg-amber-400 rounded-full animate-pulse" title="Translation engine: limited mode" />
        )}
      </button>
      ...
      {isOpen && (
        <div
          className={`absolute ${isRTL ? 'left-0' : 'right-0'} mt-2 w-64 bg-slate-900 border border-slate-800 rounded-xl shadow-2xl z-50 overflow-hidden`}
          role="listbox"
        ...
        >
          ...
          <div className="max-h-72 overflow-y-auto p-1">
            ...
            {filteredLanguages.length === 0 && (
              <p className="text-slate-500 text-xs text-center py-3">No languages found</p>
            )}
          </div>
          ...
          <div className="px-3 py-2 border-t border-slate-800 text-[10px] text-slate-500">
            ...
            {translationStatus === 'degraded' && (
              <span className="ml-2 text-amber-400">• Dynamic translation: limited</span>
            )}
          </div>
        </div>
      )}
