...
export const AdminApplicationsView = ({ adminUsername }: Props) => {
  ...
  const handleRequestKycDoc = (docName?: string) => {
    ...
    const newNotification = {
      id: `REQ-${Date.now()}`,
      appId: selectedApp.id || 'JS-2026-8802',
      schemeName: selectedApp.service || 'National Apprenticeship Training Scheme (NATS)',
      deptName: selectedApp.department || 'Ministry of Education',
      targetCitizenName: selectedApp.citizenName || 'Hriday Bardia',
      targetUidLast4: (selectedApp.citizenId || '1405').slice(-4),
      citizenId: selectedApp.citizenId || '1111 2222 1405',
