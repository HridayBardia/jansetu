...
export const AdminApplicationsView = () => {
  ...
  const adminApps = user?.username === 'dishita'
    ? mockGlobalApplications.filter(app => ['CIT-8921-HRD', 'CIT-4412-VRD'].includes(app.citizenId))
    : user?.username === 'jyoti'
    ? mockGlobalApplications.filter(app => ['CIT-7734-AYH', 'CIT-1198-STW'].includes(app.citizenId))
    : mockGlobalApplications;

  const filteredApps = adminApps.filter(app => {
    if (filter === 'All') return true;
    if (filter === 'Action Required') return app.status === 'Action Required';
    if (filter === 'SLA Breach') return app.sla.includes('Breach');
    return true;
  });
            ...
            <tbody className="divide-y divide-slate-800/50">
              {filteredApps.map(app => (
                <tr key={app.id} className="hover:bg-slate-800/30 transition-colors">
                  <td className="px-6 py-4 font-mono text-xs text-slate-200 font-bold">{app.id}</td>
                  <td className="px-6 py-4">
                    <div className="font-medium text-slate-200">{app.citizen}</div>
                  ...
                  <td className="px-6 py-4">
                    <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${
                      ...
                      app.status === 'Action Required' ? 'bg-red-500/10 text-red-400 border border-red-500/20 animate-pulse' :
                  ...
                  <td className="px-6 py-4">
                    <span className={`flex items-center gap-1 text-xs ${
                      app.sla.includes('Breach') ? 'text-red-400 font-bold' : 'text-emerald-400'
                    }`}>
                      {app.sla.includes('Breach') ? <AlertTriangle className="w-3 h-3" /> : <CheckCircle2 className="w-3 h-3" />}
                      {app.sla}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-xs text-slate-400">{app.lastUpdated}</td>
          ...
          <div className="bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="p-6 border-b border-slate-800 flex justify-between items-start">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-1">{selectedApp.service} - {selectedApp.citizen}</p>
              </div>
              <button 
            ...
            <div className="p-6 space-y-6">
              <div className="relative border-l border-slate-800 ml-3 pl-6 space-y-6 text-sm">
                ...
                <div className="relative">
                  <div className={`absolute -left-[31px] top-1 w-3 h-3 rounded-full border border-slate-900 ${selectedApp.status === 'Completed' ? 'bg-emerald-500' : 'bg-amber-500'}`} />
                  <div>
                    <h4 className={`font-bold text-sm ${selectedApp.status === 'Completed' ? 'text-emerald-400' : 'text-amber-400'}`}>Department Verification</h4>
                    <p className="text-xs text-slate-400 mt-1">
                      {selectedApp.status === 'Completed' ? 'Verified by department official.' : 'Currently pending verification.'}
                    </p>
                  </div>
                </div>
                ...
                {selectedApp.status === 'Action Required' && (
                  <div className="relative">
                    <div className="absolute -left-[31px] top-1 w-3 h-3 rounded-full bg-red-500 border border-slate-900 animate-pulse" />
                    <div>
                      <h4 className="font-bold text-red-400 text-sm">Exception: Information Request</h4>
                      <p className="text-xs text-slate-400 mt-1">Citizen needs to upload additional proof.</p>
                    </div>
                  </div>
                )}
